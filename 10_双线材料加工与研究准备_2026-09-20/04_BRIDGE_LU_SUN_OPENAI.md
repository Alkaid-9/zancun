...
五个 bridge 从不同维度攻击这个问题：B1/B2/B3 是独立输入线，B4/B5 是受门禁的 downstream synthesis。
...
这不是"三篇论文的加法"，而是专门去找**组合边界**——B1≠B3、B2≠B1、B3≠B2 这三条关键约束（见下文"概览：五个 Bridge 的交叉依赖"）各自成立，不代表把三者接在一条流水线上时接口处依然成立。

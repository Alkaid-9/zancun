...
| `docs/USER_MANUAL.md` | 隐含"可信操作手册" | **CONTAMINATED**（§三.1 EdgeIM 挂载示例，两处独立污染同址叠加） / **TOY_TESTED**（其余 API 示例） | §三.1 有两处独立污染：`:85-89` 的 `title`/`abstract` 字段本体错误（污染源 A）+ `:84` 的 `uri` 字段年份编造（污染源 D，第二轮新发现，`edge_im_2020` 应为 `edge_im`/真实年 2025）；两处需分别修复，修 A 不会带修 D；Moraine/LoopX 章节的 API 示例本身准确对应源码行为 |
...
| `docs/MULTI_KERNEL_SYSTEM_ARCHITECTURE.md` | "生产级统一控制底座" | **AGENT_GENERATED（DRAFT 级别的设计文档）** + **CONTAMINATED**（:70 契约示例、:87 虚拟路径年份） | 架构设计本身是对四个真实外部项目机制的合理转化构想，但"生产级"措辞超出目前只有 toy 原型的证据强度；科研台职责定义（§四，:144）继承污染源 A；**第二轮新发现**：:70 调度内核 JSON 契约示例硬编码 `"deadlock_detected": true` 为默认输出、:87 虚拟路径沿用编造年份，均属污染源 D，是本文件内独立于 §四 的第二处污染点 |
...
| `docs/NEXT_PHASE_EXECUTION_PLAN.md` | "下阶段演进与下周实操路线图" | **AGENT_GENERATED, PARTIALLY_MISALIGNED** | Week 1（Moraine）与 Week 3（LoopX Hooks）目标建立在 SOURCE_VERIFIED 材料上，可执行；Week 2/4（10 篇论文入库、并发死锁挖掘）建立在污染源 A/B 之上，需要先修正范围定义才能执行；**第二轮新发现**：:75 虚拟路径年份编造（污染源 D）+ :78 把 "Lock Matrix" 列为未来所有论文 L1 提纲**必须**收录项——后者是本文件里风险最高的一条，会把污染从"当前描述错误"固化为"下一步批量生成新内容时的强制规范"，修复须在启动 Week 2/4 前完成，否则会产生新的、范围更大的污染 |
| `MASTER_EVALUATION_MATRIX.md` | "28 仓全景总决算" | **AGENT_GENERATED**（元数据部分标 SOURCE_FACT，处置建议标 RECONSTRUCTION） | Stars/license 等结构化字段来自 `BATCH_METADATA_RECEIPT.json`；第二轮抽查（7/28，非全量）：WebSearch 交叉核对 TauricResearch/TradingAgents、ayghri/i-have-adhd、paperclipai/paperclip、K-Dense-AI/scientific-agent-skills、huangruiteng/loopx、headroomlabs-ai/headroom、volcengine/OpenViking 七仓星标，均落在实时波动合理范围内，未发现矛盾（诚实记录负向结果）；但抽查用聚合站快照非直连 GitHub API，证据强度是佐证而非定论，仍有 21/28 未查，不能外推到全表；28 仓处置建议本身内部自洽未见矛盾。附带发现：`huangruiteng/loopx` 已于 2026-09-19（审计前一天）迁移至 `loopx-project/loopx`（v1.1.0 组织迁移），属引用新鲜度问题非造假，见 TRUTH_AUDIT_LEDGER.tsv L23 |

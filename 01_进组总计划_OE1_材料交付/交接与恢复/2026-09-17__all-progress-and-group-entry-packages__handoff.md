...
> **2026-09-20 订正**：本档为历史快照，不是当前状态验收。BR-1“部分确认”已撤回；α/β 旧稿均不再直接使用。进组急用从 [修复入口](../README_进组急用.md) 进入，学习合同从 [本仓 v3](../../02_四论文Ownership主线/00_共享合同_计划_验收/计划与决策/2026-09-13__research__four-paper-full-ownership-v3-sol-execution-contract.md) 恢复。下方原 MyResearch 路径仅用于来源追溯。
...
6. OE-1 的旧分支选择已被 09-20 修订取代；从当前稿核本人字段、学习/贡献证据和最终副本，不由恢复者代填或发送。
...
项目仍远未“全部完成”：EdgeIM 当前六项恢复包未做完；ownership-v3 课程已建但 rubric 未冻结、用户能力未验收；P 批六包和 EmoAgent 尚未正式开考；ra26／ra28 是 Agent 先行后的逆向训练债；ra2716／ra30 未闭；OE-1 当时未选择和发送（09-20 已撤回旧分支，未改变未发状态）；进组拓展 W1 只完成一部分；三台旧科研驾驶舱仍有保存 RED，NotEMD AI 抽取和真实 LearnGraph 学习也未验收。
...
- OE-1 历史有 α／β 两稿；09-20 复核撤回 BR-1“部分确认”，转向不依赖项目获批的修订稿。个人字段、贡献核实与用户发送决定仍开放；本档不证明已发送。
...
- OE-1：09-17 历史草稿备好不等于可投递；09-20 已替换旧分支，个人字段、贡献证据、最终复核与投递决定仍开放；
...
4. OE-1 线上，使用 09-20 修订稿，补齐真实身份与本人证据、完成发前复核后才形成候选终稿；发送必须由用户另行决定。
